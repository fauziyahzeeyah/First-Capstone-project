...
function Mobile(artistes) {
  const container = document.querySelector('#mobile_skpeakers');
  const artisteCards = container.querySelectorAll('.mobile-artiste');
  for (let i = 0; i < artistes.length; i += 1) {
    const artisteData = artistes[i];
    const artisteCard = artisteCards[i].querySelector('img');
    artisteCard.src = artisteData.img;
    artisteCard.alt = artisteData.name;
    const content = artisteCards[i].querySelector('p');
    content.innerHTML = `
